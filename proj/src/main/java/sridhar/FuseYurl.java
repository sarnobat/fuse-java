...
public class FuseYurl extends FuseFilesystemAdapterFull {
    ...
    @Deprecated
    private static final String string = System.getProperty("user.home") + "/github/fuse-java/yurl";
...
//                        @Override
        ...
        Collection<String> s2 = FluentIterable.from(dirId2Urls.get(currentDirId))
                .transform(new Function<Object, String>() {
                    ...
                    // @Override
                    public String apply(Object input) {
                        String string = ((String) input).replace("http://", "").replace("https://", "")
                                .replace("www.", "").replace("-", " ").replace("_", " ").replaceFirst("/", " - ")
                                .replace("/", "_") + ".url";
                        return string;
                    }
                }).toList();
